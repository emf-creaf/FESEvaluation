...
author: "M. De Cáceres, J.V. Roces Diaz, J. Martinez-Vilalta, J. Vayreda, M. Banqué, F. D’Adamo, J.M. Espelta"
date: "2023-07-07"
...
The present technical document provides definitions and operational estimation procedures for a set of forest ecosystem services to be evaluated for Catalonia both in historical assessments (using forest inventory data) and future projections (using model simulations of forest function and dynamics). The following table has been modified from @roces-diaz_assessing_2018, according to changes in definitions and indicators decided after that publication.
...
Keeping in mind the model output related to each metric and its range value in the Catalan forest, we will present our participants with a questionnaire where we ask them to simply draw what they think is the shape of the function that best relates each model output to the potential recreational value, scored between 0 and 1. Each questionnaire respondent produces a potential recreational value between 0 and 1 for each of the seven model metrics (i.e., 5 stand-level and 2 landscape-level). We assume four types of relationships, i.e., linear increase, linear decline, V-shaped relationship, and inverted V-shaped relationship. The answers of the questionnaire respondents should be aggregated for each metric into a single function. Then, the seven different functions should combined into an overall potential recreational value. Different options are available for this, including their sum, multiplication, and PCA, among others, with the possibility to apply different weights. 

**Potential improvements**: The idea was to include also the amount of natural deadwood. In projections of forest dynamics, inclusion of species richness into this indicator requires that the model incorporates colonization processes. Otherwise, local extinction will allways decrease species richness.
